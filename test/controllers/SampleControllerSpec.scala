...
class SampleControllerSpec extends PlaySpec with Results with MockitoSugar {
    "SampleController#findByName" should {
        "return formatted name" in {
            ...
            val controller = new SampleController(mockUserService, stubMessagesControllerComponents())(ExecutionContext.global)
    ...
    "SampleController#findAll" should {
        "return all users as JSON" in {
            ...
            val controller = new SampleController(userService, stubMessagesControllerComponents())(ExecutionContext.global)

            // when
            when(userService.findAll()).thenReturn(Future.successful("1: Alice\n2: Bob\n3: Carol"))
        ...
        "handle errors gracefully" in {
            ...
            // when
            when(userService.findAll()).thenReturn(Future.failed(new Exception("Database error")))
    ...
    "SampleController#formatNames" should {
        "format a single name correctly" in {
            // setup
            val controller = new SampleController(mock[UserService], stubMessagesControllerComponents())(ExecutionContext.global)
