...
object SampleObject extends App {
     ...
     * 【クラス・トレイト】
     */
    private val sampleClass: SampleClass = new SampleClass("sampleMethod", "sampleArg")
    sampleClass.draw()
     ...
     */
    // パターン1(apply)
    private val specialResult: Int = SpecialObject(1, 2)
    println(specialResult)
    // パターン2(unary)
    private val sampleClass2: SampleClass = new SampleClass("sampleClass2", "sampleArg2")
    println(!sampleClass2)

    /**
     * 【caseクラス】
     */
    private val sampleCase: SampleCaseClass = SampleCaseClass("sampleArg1", "sampleArg2")
    sampleCase match {
